...
def test_multi_variant(valid_multi_variant):
    entrez_client = MockEntrezClient(valid_multi_variant)
...
def test_missing_variant(invalid_variant):
    ...
    result = NcbiSnpClient(entrez_client).hgvs_from_rsid("rs123456789")
    assert result == {}
...
def test_non_rsid():
    ...
    with pytest.raises(ValueError):
        NcbiSnpClient(entrez_client).hgvs_from_rsid(["not a rsid"])

    with pytest.raises(ValueError):
        NcbiSnpClient(entrez_client).hgvs_from_rsid(["rs1a2b"])

    with pytest.raises(ValueError):
        NcbiSnpClient(entrez_client).hgvs_from_rsid(["12345"])
